...
async function main() {
  ...
  const tagalogFaqs = data.faqs.filter(f => f.category === 'tagalog');

  if (tagalogFaqs.length === 0) {
    console.log('No Tagalog FAQs found');
    process.exit(1);
  }

  console.log(`Found ${tagalogFaqs.length} Tagalog FAQ entries. Seeding...\n`);
  ...
  const ids = await bot.seedFAQs(tagalogFaqs);

  console.log(`\nSeeded ${ids.length} Tagalog FAQ entries:`);
  tagalogFaqs.forEach((faq, i) => {
